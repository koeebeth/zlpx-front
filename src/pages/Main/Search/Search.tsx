  ...
  return (
    <>
      ...
      <div className="h-12 w-screen p-2 px-4 gap-2 overflow-hidden flex">
        <div className={`h-full w-full ${!searchFocus && "shrink-0"} bg-zinc-300 rounded-md flex items-center justify-start pl-2 transition-all ease-out`}>
        ...
        <button onClick={() => setSearchFocus(false)} className={`text-light-purple animate-slide inline transition-all duration-200 ease-out ${!searchFocus && "opacity-5" }`}>Отмена</button>
      </div>
      {searchFocus ? (
        <Results />
      ) : (
